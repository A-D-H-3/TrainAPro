api.controller = function ($scope, $rootScope, spUtil, $timeout) {
  ...
  $scope.currentClass = function (id, when, type) {
    $timeout(function () {
      classID = id;
      ...
      broadcating(id, modifiableRecord, type);
        
      },
      1);
  }

  function broadcating(id, modifiableRecord, type) {
  ...
  for (var i = 0; i < c.data.classes.length; i++) {
    ...
    if (c.data.classes[i].when === "active") {
      ...
      $scope.currentClass(c.data.classes[i].sys_id, "active", c.data.classes[i].type);
    }
